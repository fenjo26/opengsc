...
//   3. the first connected account (if any)
//   4. the global default key                (seoKey_<engine>)
"use client";
...
export function resolveEngineKey(engine: Engine, siteDbId: string): string {
  ...
  if (sel === "custom") {
    return localStorage.getItem(`seoKey_${engine}_${siteDbId}`) || globalKey || accounts[0]?.key || "";
  }
  if (sel) {
    const acc = accounts.find(a => a.id === sel);
    if (acc?.key) return acc.key;
  }
  // No explicit per-site choice: prefer the global default, else the first account.
  return globalKey || accounts[0]?.key || "";
}
