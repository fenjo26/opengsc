...
- ОБЪЁМ секции — строго в её word_count-диапазоне: не короче и не длиннее. Это объём ИМЕННО этой секции (для H2 — только его вступление, БЕЗ подсекций: у каждого H3 свой word_count). Без воды и повторов.
- ПОДАЧА (ПРИОРИТЕТ НАД copywriter_notes): основа — связные абзацы по 2-4 предложения. На ВСЕ секции этого задания — максимум ОДИН список, и только в процедурной секции (регистрация/шаги/чек-лист); в остальных — ТОЛЬКО проза. Если copywriter_notes секции предлагают список или таблицу, а эти правила запрещают — ИГНОРИРУЙ подсказку и пиши прозой: содержание бери из notes, формат — отсюда.
