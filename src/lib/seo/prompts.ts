...
export function buildOutlinePrompt(args: {
  ...
  const factsBlock = compFacts
    ? `\n\nРЕАЛЬНЫЙ КОНТЕНТ КОНКУРЕНТОВ ИЗ ТОПА ВЫДАЧИ (ЕДИНСТВЕННЫЙ допустимый источник проверяемой конкретики): используй ЭТИ тексты как опору для всех конкретных значений — точные цены/суммы, названия моделей/версий/изданий, характеристики, объёмы памяти, даты, имена игр/товаров/ритейлеров. ПРАВИЛО ЗАЗЕМЛЕНИЯ: любую такую конкретику бери ТОЛЬКО отсюда (или из ручных текстов). Если значения здесь НЕТ — категорически НЕ придумывай его: в summary/copywriter_notes дай качественный ориентир или диапазон и явную пометку копирайтеру «точное значение взять из источников при написании; не выдумывать». НЕ ставь фейковую точную цифру и НЕ вставляй токен-плейсхолдер в заголовки/summary. Цены — в валюте региона. Текст не копируй дословно — извлекай факты.\n${compFacts}`
    : `\n\nЗАЗЕМЛЕНИЕ (текстов конкурентов нет): у тебя НЕТ подтверждённого источника проверяемой конкретики. Поэтому НЕ выдумывай точные цены, названия моделей/версий/изданий, спеки, объёмы памяти и даты. Держи такие места качественными (диапазон/ориентир) и в copywriter_notes помечай «точное значение взять из источников при написании». Структуру, сущности и ключи раскрывай полноценно — ограничение касается ТОЛЬКО придуманных конкретных значений.`;
...
- ПЛОТНОСТЬ ЗАГОЛОВКОВ: масштабируй под объём ~1 заголовок на 60-100 слов. При целевом объёме ~1500 слов дай 14-20 секций (H2+H3 суммарно), при ≥1800 — 18-24. Соотношение примерно 1 H2 на 2-3 H3. Покрой ВСЕ реальные sub-intents и под-вопросы (включая узкие: конкретные маршруты, цены по типам, FAQ-вопросы как H3), а не 4-6 общих блоков.
- СУЩНОСТИ: для крупных секций 5-9 сущностей с весами, для мелких H3 — 2-4. НЕ 1-2 на секцию.
...
- SUMMARY: 3-5 предложений с числовыми ориентирами (расстояние в км, время в мин/ч, диапазоны цен в валюте) — но ТОЛЬКО взятыми из текстов конкурентов/ручных текстов. Если точного значения в источниках нет — дай диапазон/качественный ориентир и пометь «уточнить из источников при написании», НЕ выдумывай точную цифру. Не общие фразы, но и не фейковая точность.
...
- ENTITY_ANALYSIS: заполни ПОЛНОСТЬЮ и богато — content_strategy (все 4 подблока), primary_entity (атрибуты+триплеты+authority_validation), supporting_entities (4-6 сущностей), keyword_strategy (primary/lsi/long_tail с usage), visual_elements (с prompt). Это отдельная ценность отчёта — не оставляй пустым и не сокращай.
...
ЖЁСТКИЕ ПРАВИЛА:
- СТРУКТУРА И ИЕРАРХИЯ (КРИТИЧНО): ровно ОДИН H1 (из Title/мета-блока). КАЖДАЯ секция структуры обязана стать заголовком РОВНО своего уровня: h_level=H2 → «## », H3 → «### », H4 → «#### ». НЕ повышай, не понижай, не объединяй, не пропускай и не переставляй заголовки; НЕ превращай заголовок секции в обычный абзац и НЕ сливай первую секцию с H1. Порядок секций — как в структуре. Если H1 и первый H2 похожи — всё равно оставь оба отдельными заголовками.
- ${NO_FABRICATION}
...
Статусы:
- confirmed: подтверждается источниками ИЛИ это бесспорный общеизвестный факт, который НЕ противоречит источникам.
- partial: данные расходятся/неточны (цена близкая, но иная; регион/валюта отличаются; диапазон не совпал).
- unconfirmed: ПРОТИВОРЕЧИТ источникам, ЛИБО это конкретная цифра/цена/дата/модель/спека, которой НЕТ ни в источниках, ни в общеизвестном (вероятно выдумана). ТОЛЬКО такие подлежат правке.
Важно: НЕ помечай unconfirmed правдоподобный общеизвестный факт лишь потому, что его нет в сниппетах.
