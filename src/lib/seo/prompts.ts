...
- "visual_elements": суммарно по статье пометь 2-4 САМЫЕ табличные секции (сравнения, цены/бонусы, платёжные методы, характеристики/RTP) элементом { "type":"table", "title":"", "description":"какие колонки и данные" }; у остальных секций — пустой массив.
...
export function buildSectionTextPrompt(args: {
  ...
  allowList?: boolean;      // only the procedural chunk may render ONE list
...
- ПОДАЧА (ПРИОРИТЕТ НАД copywriter_notes): ТОЛЬКО связные абзацы по 2-4 предложения.${args.allowList ? " Исключение: в процедурной секции (регистрация/пошаговая инструкция) сделай ОДИН нумерованный список шагов — единственный список на всё задание." : " СПИСКИ ЗАПРЕЩЕНЫ в этих секциях — перечисления оформляй прозой внутри предложений («доступны X, Y и Z»)."} Если copywriter_notes предлагают списки — ИГНОРИРУЙ: содержание из notes, формат отсюда.
