...
export function buildAutoFactCleanPrompt(args: { article: string; factsBank: string; language: string }): string {
  return `Ты — фактчек-редактор. Ниже ГОТОВАЯ СТАТЬЯ (Markdown) и БАНК ФАКТОВ — проверенные факты, извлечённые из топ-источников выдачи, на которых статья и строилась. Сверь конкретные утверждения статьи с банком и сделай её фактически чистой, СОХРАНИВ объём, структуру и стиль.

Действуй точечно:
1. Если число/цена/спека/дата в статье ПРОТИВОРЕЧИТ банку — исправь по банку.
2. Если конкретика ЯВНО выдумана и не поддержана ни банком, ни общеизвестным — убери или обобщи (без точной цифры).
3. Правдоподобные и общеизвестные факты, которых просто нет в банке, — ОСТАВЬ (банк неполный, «нет в банке» ≠ «неправда»).
4. СИНХРОНИЗАЦИЯ ЧИСЕЛ: если одно и то же значение встречается в прозе и в таблице/списке — приведи их к ОДНОМУ значению (не оставляй, где в тексте одно, а в таблице другое).
ЗАПРЕЩЕНО: маркеры [ПРОВЕРИТЬ], плейсхолдеры, просьбы к читателю проверить, выдуманные числа. Не сокращай статью, не трогай заголовки и их порядок, сохрани мета-блок в начале (если есть), пиши на языке статьи (${args.language}).
...
ЖЁСТКИЕ ПРАВИЛА:
- СТРУКТУРА И ИЕРАРХИЯ (КРИТИЧНО): ровно ОДИН H1 (из Title/мета-блока). H1 должен ДОСЛОВНО совпадать с Title из мета-блока (включая год). СРАЗУ после H1 идёт ПЕРВЫЙ H2 — БЕЗ вводного абзаца/лида между ними. НЕ пиши НИКАКОГО текста между H1 и первым «## ». Весь текст статьи — ТОЛЬКО под заголовками H2/H3 (никаких «висячих» абзацев под H1). Вступление, если нужно, помещай ПОД первым H2 (он обычно и есть вводная/обзорная секция из структуры). КАЖДАЯ секция структуры обязана стать заголовком РОВНО своего уровня: h_level=H2 → «## », H3 → «### », H4 → «#### ». НЕ повышай, не понижай, не объединяй, не пропускай и не переставляй заголовки; НЕ превращай заголовок секции в обычный абзац и НЕ сливай первую секцию с H1. Порядок секций — как в структуре.
